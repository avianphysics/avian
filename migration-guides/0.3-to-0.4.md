...
**Avian 0.4** contains a _lot_ of changes and improvements for joints.
...
`ContactPoint::normal_impulse` previously corresponded to the clamped accumulated normal impulse from the last substep, used for warm starting the contact solver. It did _not_ represent the total impulse applied across substeps and restitution, despite the ambiguous name.
