...
or for lower level control, use the `ColliderTrees` resource directly. If you get query conflicts
because `SpatialQuery` queries for `&Position` and `&Rotation` and your system also queries for them mutably,
consider deferring the mutation to a later point to avoid the conflict.

Shape casts now also return hits in arbitrary order when `max_hits > 1`, similar to ray casts.
This is *much* more efficient.

Previously, `point2` and `normal2` in `ShapeHitData` were in local space, even though the docs describe them
as being in world space. This has been fixed, and they are now in world space like the other values.
